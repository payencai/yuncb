...
public class AccountManageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int pos=0;
        List<MyAccount> accountList=LitePal.findAll(MyAccount.class);
        for (int i = 0; i <accountList.size() ; i++) {
            MyAccount account=accountList.get(i);
            if(MyApplication.getUserInfo().getUsername().equals(account.getPhone())){
        ...
        mAccountAdapter.setOnDeleteClickListener(new MyPublishListAdapter.onDeleteClickListener() {
            ...
            public void onClick(int pos) {
                if(MyApplication.getUserInfo().getUsername().equals(mAccounts.get(pos).getPhone())){
                    ToastUtil.showToast(AccountManageActivity.this,"当前账号不能删除");
                    return;
                }
                MyAccount myAccount=mAccounts.get(pos);
                mAccounts.remove(pos);
                mAccountAdapter.notifyDataSetChanged();
                LitePal.deleteAll(MyAccount.class, "phone = ?", myAccount.getPhone());
            }

            @Override
            public void onItemClick(int position) {
                if(MyApplication.getUserInfo().getUsername().equals(mAccounts.get(position).getPhone())){
                    return;
                }
                mAccountAdapter.setPos(position);
                mAccountAdapter.notifyDataSetChanged();
                MyApplication.setUserInfo(null);
                MyApplication.token="";
                MyApplication.setIsLogin(false);
                RongIM.getInstance().logout();
                loginByPwd(mAccounts.get(position).getPhone(),mAccounts.get(position).getPassword());
    ...
    private void loginByPwd(String account,String pwd){
        ...
        String rsaPwd=null;
        try {
            rsaUsername = RSAUtil.encrypt(account);
            rsaPwd= RSAUtil.encrypt(pwd);
        } catch (Exception e) {
            e.printStackTrace();
        }
        params.put("username", rsaUsername);
        params.put("password", rsaPwd);
