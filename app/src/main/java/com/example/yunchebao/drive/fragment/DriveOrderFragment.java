...
public class DriveOrderFragment extends Fragment {
   ...
   @BindView(R.id.refresh)
    ...
    private void initView() {
        ...
        mYuedanAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            @Override
        ...
        HttpProxy.obtain().get(PlatformContans.SubstituteDriving.getSubstituteDrivingOrderListByUser, params, token, new ICallBack() {
            ...
            public void OnSuccess(String result) {
                ...
                try {
                    ...
                    List<ReplaceOrder> replaceOrders=new ArrayList<>();
                    for (int i = 0; i < data.length(); i++) {
                        JSONObject item = data.getJSONObject(i);
                        ReplaceOrder yueOrder = new Gson().fromJson(item.toString(), ReplaceOrder.class);
                        replaceOrders.add(yueOrder);
                    }
                    mYuedanAdapter.addData(replaceOrders);
                    if(isLoadMore){
                        if(data.length()==0){
                            mYuedanAdapter.loadMoreEnd(true);
                        }else{
                            mYuedanAdapter.loadMoreComplete();
                        }
                    }else{
                        mYuedanAdapter.loadMoreComplete();
                    }
