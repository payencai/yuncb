...
public class RentCarOrderActivity extends AppCompatActivity {
    ...
    private void postOrder() {
        ...
        String phoneNumber = rentCarPersonPhoneView.getText().toString().trim();
        ...
        String callTelephone = rentCarPersonContactPersonPhoneView.getText().toString().trim();
        ...
        if(callTelephone.equals(phoneNumber)){
            ToastUtil.showToast(this,"2个手机号不能一样！");
            rentCarPersonIdentifyView.requestFocus();
            return ;
        }
        if(callName.equals(name)){
            ToastUtil.showToast(this,"紧急联系人和姓名不能相同");
            rentCarPersonIdentifyView.requestFocus();
            return ;
        }
